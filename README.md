...
운영 담당자가 Google Sheets에 지원자를 미리 등록하면 지원자는 이름, 이메일, 휴대폰 번호로 본인 확인 후 검사에 참여합니다. 현재 기본 운영 버전은 전체 320문항 중 지원자별로 250문항을 구성해 출제하는 `v2-bank-pilot`입니다.
...
| 구분 | 전체 문항 | 개인별 출제 |
| --- | ---: | ---: |
| 핵심 Culture Fit 5개 영역 (원칙, 혁신, 고객, 소통, 도전) | 200 | 150 (영역별 30) |
| **Team-Fit (팀 적합도 및 협업 성향)** | **20** | **20** |
...
같은 세션에는 항상 같은 문항 조합이 재현됩니다. 다른 세션에는 영역별 문항 수를 유지하면서 다른 조합이 출제될 수 있습니다. 일관성 기준문항 10개, 반복문항 10개, IMC 5개, Team-Fit 20개는 항상 포함됩니다.

### 채점 원칙

- 총점에는 선별된 핵심 Culture Fit 150문항만 반영합니다.
- **Team-Fit (팀 적합도)**, OCB, CWB, 정직성·무결성은 별도 보조척도로 계산되어 면접관 리포트에 행동 프로파일 및 맞춤 면접 질문과 함께 제공됩니다.
