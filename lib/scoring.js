...
/**
 * OCB/CWB 도메인(최근 12개월 행동빈도) 문항인지 확인
 */
export function isNoExperienceEligible(item) {
  const domain = String(item?.domain || '');
  return domain.includes('OCB') || domain.includes('CWB');
}
...
export function normalizeAnswers(answers = {}, items = loadFullItems()) {
  ...
  for (const [itemId, rawValue] of Object.entries(answers || {})) {
    ...
    // 값 0 = "해당 경험 없음" — OCB/CWB 도메인에서만 허용
    if (value === 0 && isNoExperienceEligible(item)) {
      normalized[itemId] = 0;
      noExperienceKeys.push(itemId);
      continue;
    }
  ...
  return { answers: normalized, invalidKeys, noExperienceKeys };
}
...
export function calculateAssessmentScore(answers = {}, meta = {}) {
  ...
  const facetTotals = {}; // { [`${domain}::${facet}`]: { sum: 0, count: 0, domain, facet } }
  ...
  for (const item of items) {
    ...
    // 값 0 = "해당 경험 없음" — 응답은 했지만 점수 합산에서 제외
    if (numVal === 0) {
      const domain = item.domain || '기타';
      if (!domainTotals[domain]) {
        domainTotals[domain] = { sum: 0, count: 0, noExperienceCount: 0 };
      }
      domainTotals[domain].noExperienceCount = (domainTotals[domain].noExperienceCount || 0) + 1;
      continue;
    }
    ...
    // Facet별 점수 집계 (팀적합도 등 하위 영역 분석용)
    if (item.facet) {
      const facetKey = `${domain}::${item.facet}`;
      if (!facetTotals[facetKey]) {
        facetTotals[facetKey] = { sum: 0, count: 0, domain, facet: item.facet };
      }
      facetTotals[facetKey].sum += scoredVal;
  ...
  for (const [dom, data] of Object.entries(domainTotals)) {
    const noExp = data.noExperienceCount || 0;
    if (data.count > 0) {
      const avg = Number((data.sum / data.count).toFixed(2));
      const pct = Math.round(((data.sum - data.count) / (data.count * 4)) * 100);
      domainScores[dom] = { average: avg, scorePercent: pct, count: data.count, noExperienceCount: noExp };
      ...
      domainScores[dom] = { average: null, scorePercent: null, count: 0, noExperienceCount: noExp };
    }
  }

  // Facet별 점수 산출
  const facetScores = {};
  for (const [fKey, data] of Object.entries(facetTotals)) {
    if (data.count > 0) {
      facetScores[fKey] = {
        domain: data.domain,
        facet: data.facet,
