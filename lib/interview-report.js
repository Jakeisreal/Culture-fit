 ...
 * 종합 면접 리포트 데이터 생성 함수
 */
export function generateInterviewReport(sessionRecord = {}, scoreResult = {}, responseQuality = null, cohortStats = null) {
  ...
  // 1. 컬쳐핏 5대 영역 프로파일 구축 (응시자 집단 평균 우선 적용)
  const cultureFitProfiles = Object.values(CULTURE_FIT_DOMAINS).map((def) => {
    ...
    // 실제 응시자 집단 평균이 있으면 우선 사용, 없으면 사전 규준 평균 사용
    const normMean = (cohortStats?.cultureNormMeans && typeof cohortStats.cultureNormMeans[def.key] === 'number')
      ? cohortStats.cultureNormMeans[def.key]
      : def.normMean;
  ...
  // 3. 팀핏 4대 하위 영역 프로파일 구축 (응시자 집단 평균 우선 적용)
  const teamFitProfiles = Object.entries(TEAM_FIT_DOMAINS).map(([facetKey, def]) => {
    ...
    // 실제 응시자 집단 평균이 있으면 우선 사용, 없으면 사전 규준 평균 사용
    const normMean = (cohortStats?.teamNormMeans && typeof cohortStats.teamNormMeans[facetKey] === 'number')
      ? cohortStats.teamNormMeans[facetKey]
      : def.normMean;
    const diff = average != null ? Number((average - normMean).toFixed(2)) : 0;
    const level = getScoreLevel(average, normMean);
  ...
  // 5-1. 검사 응답 의심 뱃지 표기 사유 추출 (응답 속도가 너무 빠른 경우 제외)
  const cautionReasons = [];

  const imcCount = Number(actualScoreResult.imcFailedCount) || 0;
  if (imcCount > 0) {
    cautionReasons.push(`주의력검사(IMC) 실패: 문항 지시문과 다른 번호 응답 (${imcCount}건 감지)`);
  }

  const largeDiffPairs = Number(actualScoreResult.consistency?.largeDifferencePairs) || 0;
  if (largeDiffPairs >= 4 || actualScoreResult.flags?.includes('RESPONSE_INCONSISTENCY_REVIEW')) {
    cautionReasons.push(`중복문항 불일치: 동일·유사 문항 간 2.5점 이상의 상반된 응답 패턴${largeDiffPairs > 0 ? ` (${largeDiffPairs}쌍)` : ''}`);
  }

  if (actualScoreResult.flags?.includes('REVERSE_INCONSISTENCY')) {
    cautionReasons.push('역문항 응답 불일치: 정방향과 역방향 문항 간 유의미한 일관성 모순 감지');
  }

  const isExtreme = actualScoreResult.flags?.includes('UNIFORM_RESPONSE') || actualScoreResult.flags?.includes('FAST_RESPONSE_EXTREME');
  if (isExtreme) {
    cautionReasons.push('전 영역 극단값/균일 패턴: 거의 모든 문항을 동일하거나 극단적 번호로만 응답');
  }

  const sdsVal = domainScores['반응왜곡(사회적바람직성)']?.average;
  if ((sdsVal != null && sdsVal >= 4.2) || actualScoreResult.flags?.includes('HIGH_RESPONSE_DISTORTION')) {
    cautionReasons.push(`사회적 바람직성 과다: 도덕적·규범적으로 자신을 지나치게 긍정 포장 (점수: ${typeof sdsVal === 'number' ? sdsVal.toFixed(2) : '-'} / 기준: 4.20)`);
  }

  const imVal = domainScores['반응왜곡(인상관리)']?.average;
  if (imVal != null && imVal >= 4.0) {
    cautionReasons.push(`인상관리(IM) 과다: 타인 평가를 의식하여 의도적으로 과장 응답 (점수: ${imVal.toFixed(2)} / 기준: 4.00)`);
  }

  const sdeVal = domainScores['반응왜곡(자기기만)']?.average;
  if (sdeVal != null && sdeVal >= 4.2) {
    cautionReasons.push(`자기기만(SDE) 과다: 무의식적인 자기 과대평가 성향 (점수: ${sdeVal.toFixed(2)} / 기준: 4.20)`);
  }

  const cwbVal = domainScores['역기능행동(CWB)']?.average;
  if (cwbVal != null && cwbVal < 2.8) {
    cautionReasons.push(`역기능행동(CWB) 징후: 조직 규범 및 신뢰를 저해하는 행동 위험성 (점수: ${cwbVal.toFixed(2)} / 기준: 2.80 미만)`);
  }

  if (actualScoreResult.flags?.includes('EXCESSIVE_FOCUS_OUT')) {
    cautionReasons.push('비정상 화면 이탈: 검사 진행 중 화면 이탈(Focus Out) 10회 초과');
  }

  if (quality.tier === 'retest_recommended' && actualScoreResult.completionRate != null && actualScoreResult.completionRate < 0.8) {
    cautionReasons.push(`미완료 검사: 응답 완료율 80% 미만으로 신뢰도 부족 (완료율: ${Math.round(actualScoreResult.completionRate * 100)}%)`);
  }

  const hasAuthenticityWarning = cautionReasons.length > 0;
  ...
  // 백분위 및 등급: cohortStats가 제공되면 응시자 집단 내 실제 백분위/등급 사용, 없으면 사전 기준 환산
  let grade = cohortStats?.grade;
  let percentile = cohortStats?.percentile;
  ...
  const topPercent = cohortStats?.topPercent != null
    ? cohortStats.topPercent
    : Math.max(1, 100 - percentile);

  const percentileText = cohortStats?.rank && cohortStats?.cohortCount
    ? `상위 ${topPercent}% (${cohortStats.cohortCount}명 중 ${cohortStats.rank}위)`
    : `상위 ${topPercent}%`;
  ...
  const performanceMetrics = {
    ...
    topPercent,
    rank: cohortStats?.rank || null,
    grade,
    cohortCount: cohortStats?.cohortCount || null,
    isCohortBased: Boolean(cohortStats),
