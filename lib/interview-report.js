...
export const SUPPLEMENTARY_DEFINITIONS = {
  '팀적합도(Team-Fit)': {
    label: '팀적합도(Team-Fit)',
    definition: '팀의 공동 목표를 위해 동료와 적극 협력하고, 피드백을 수용하며, 갈등을 원만히 조율하여 팀에 신속히 적응하는 행동 성향',
    interviewQuestion: '팀 프로젝트나 공동 과제를 진행하면서 팀원과 의견 차이가 크게 발생했거나 협업이 어려웠던 상황에서, 이를 해결하기 위해 본인이 취한 구체적 행동과 결과는 무엇입니까?',
...
export function generateInterviewReport(sessionRecord = {}, scoreResult = {}, responseQuality = null) {
  ...
  return {
    ...
    domains: [...domainProfiles, ...supplementaryScales].map((dp) => ({
      ...
      supplementary: Boolean(dp.supplementary),
    })),
  };
}
